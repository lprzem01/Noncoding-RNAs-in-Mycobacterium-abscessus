...
6 RNA-seq datasets were used: PRJNA694147(GSE165352), PRJNA886436(GSE214640), PRJNA917642(GSE222081), PRJNA1001307(GSE239869), PRJNA313774(GSE78787), PRJNA295556(GSE7296)

These were QC checked with fastqc quality and adapter trimmed with fastp and mapped to reference genome with bwa and samtools.

## 2) Mapping coverage estimated to aid paramater selection 

Create a folder for each datset
```{r preliminaries}
#list all datasets 
...
Adapted code used to create tables and graphs depicting intergenic mapping coverage to aid with Baerhunter paramater selection.
```{r coverage_estimates}
...
#make a list of all the bam files in each dara folder
for (dataset in datasets) {
...
Following preliminary predictions using P90 for low and high coverage cutoff. Low coverage cutoff was adjusted to allow for discovery of as many known ncRNAs while limiting noise. High coverage was set to optimise the length of predictions to be as close to original annotations as possible. 
...
for (dataset in datasets) {
  ...
  for (name in gff_names) {
     ...
     #obtain an isolated table for table typw in df
     sRNA_UTR_df <- df[1]
     overlap_df <- df[2]
     counts_df <- df[3]
     unique_df <- df[4]
...
Combine generated annotation files 

1) Edit annotation file to eliminate low expression and overlapping predictions. Combine gff files into final combined gff.

2) Use new combined annotation file to re-count feature expression and make a single counts matrix for all samples for all transcripts.

3) Make a counts matrix from combination of all counts from datasets.

4) Combine annotations: filter for expression level and length, resolve sRNA, and UTR overlaps, also overlaps with annotated ncRNAs


```{r combine_annotations, eval=F}
#combining (flagged) gff3 files with predicted sRNA/UTR annotations to get one master list
...
# reduce to align ranges and merge overlapping ranges 
...
Make a counts matrix from combination of all counts.
...
control_conditions.df[order(control_conditions.df$sample), ]
...
saveRDS(dds.untransformed, file=here("Output/dds.untransformed.RData"))

conditions <- read_csv(here("Data/control_conditions_df.csv"), col_names=T)
# Create color mapping for each study
study_levels <- unique(conditions$study)
cols <- c("#66A61E", "#E7298A", "#E6AB02", "#1B9E77", "#D95F02", "#7570B3")
names(cols) <- study_levels
# Assign colors to samples based on their study
sample_colors <- cols[conditions$study]

#set up the plot area 
par(mfrow=c(1,2))
par(cex.axis=0.5) 
par(mar=c(4,2,1,1))
#plot the fisrt boxplot for non normalised data 
...
#create boxplot for log2 transforrmed data
...
#save rlog control se data 
saveRDS(rlog_control_se, file=here("Output/rlog_control_se.RData"))

#boxplot for rlog transformed data 
...
Rlog transformation subset by feature type to visualise if transformation is suitbale for ncRNAs as well as coding regions.

```{r rlog_transf_grouped, message=FALSE}
...
#add the legend in the space for the 4th plot so it does not obstruct data 
plot.new()
legend("center",
               legend = names(cols),
               fill = cols,
...
PCA analysis for non transformed data.

```{r pca_limma_rlogc}

# make PCA plot for limma batch corrected control set (limma_rlogc)
# load count matrix and conditions df
dds.untransformed <- readRDS(file=here("Output/dds.untransformed.RData"))
rlog_control_se <- readRDS(file=here("Output/rlog_control_se.RData"))
conditions <- read_csv(here("Data/control_conditions_df.csv"), col_names=T)
# Create color mapping for each study
study_levels <- unique(conditions$study)
cols <- c("#66A61E", "#E7298A", "#E6AB02", "#1B9E77", "#D95F02", "#7570B3")
...
#generating PCA table for untransformed data
...
#look at the PCa overview
summary(countdata_df)

#PCA plot for original untransformed counts
...
#generate PCA data for rlog transformed counts
...
combined_plot
ggsave(here("Results/untransformed_vs_rlog_PCA.png"), plot = combined_plot)
```
...
#batch effect removal on untransformed count data for comparission
...
```{r}
# load batch corrected data and conditions df
...
#combine th batch corrected PCA plots 
...
combined_plot
ggsave(here("Results/batch_corrected_PCA.png"), plot = combined_plot)

```

Compare with and without batch correction using hierarchical clustering dendrograms
...
# Create dendrogram
#using ward due to large amount of noise observed
...
# Reorder conditions to match dendrogram order
ordered_cond <- conditions[match(dend_labels, conditions$sample), ]
...
Following visualisation with PCA and dendrograms it has been revealed that batch effect correction was not sucessfult and an alterantive analysis aproach has been used for differential expression. 

Each dataset was independantly filtered for expression and length. 
...
```{r}
#recount all paired end datasets
datasets <- c("PRJNA694147","PRJNA886436", "PRJNA917642", "PRJNA1001307") 
for (dataset in datasets){ 
  print(dataset)
  count_features(bam_dir=here("Data", dataset),
...
for (dataset in datasets) {
  ...
  #rename colnames to only filename 
  colnames(countdata) <- gsub("\\_sorted$","",colnames(countdata))
  #remove final_ from sample names
  colnames(countdata) <- gsub("^final_", "", colnames(countdata))
  #sort samples
  ...
  dds_name <- DESeqDataSetFromMatrix(countData = as.matrix(countdata),
                                colData = control_conditions.df,
                                design = ~condition)
  # pre-filter reads to exclude rows with very low expression
  ...
  #display summary of the PCA analysis
  summary(pca_rlogc)
  #create PCA plot
  rlogc_plot <- ggplot(pca_rlogc, aes(x=PC1,y=PC2,color=condition)) +
          scale_color_manual(values = study_levels) +
          geom_point(size=3) + 
          ggtitle(paste(dataset, "PCA for normalised counts"))+
          theme_bw()
  #display and save plot
  rlogc_plot
  #create a name variable based on currently analysed dataset
...
for (dataset in datasets) {
  ...
  #create a volcano plot for each condition compared to control
  for (condition in conditions){
    ...
    #show differentially expressed putative features only 
    ...
    #create and save a plot including all features as well as one with putative features only
...
## 7) Batch correction issues exploratory analysis.  

Check if splitting paired and single end datasets helps with batch correction.

Make a combined gff file for all paired and single end datasets separately 


```{r combine_paired_annotations, eval=F}
ref <- import.gff3(here("Genome", "GCA_000069185.gff3"))
nc_gr <- ref[(elementMetadata(ref)[, "type"]=="ncRNA_gene")]
...
header <- c(header, "# produced by baerhunter")
## Create the final GFF3 file. 
output_file<-here("Output/paired_comb_filtered.gff3")
write.table(header, output_file, sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(annotation_dataframe, output_file, sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
```

```{r combine_snle_annotations, eval=F}
# import each .gff and subset by ncRNAs
ref <- import.gff3(here("Genome", "GCA_000069185.gff3"))
nc_gr <- ref[(elementMetadata(ref)[, "type"]=="ncRNA_gene")]
...
total_srnas_gr<-c(srnas_gr5, srnas_gr6)
# reduce to align ranges and merge overlapping ranges, those with specified gap between 
red_srnas_gr<-reduce(total_srnas_gr, min.gapwidth=5L)
total_utrs_gr<-c(utrs_gr5, utrs_gr6)
red_utrs_gr<-reduce(total_utrs_gr, min.gapwidth=5L)
...
## Create the final GFF3 file. 
output_file<-here("Output/single_comb_filtered.gff3")
write.table(header, output_file, sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(annotation_dataframe, output_file, sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
...
for (dataset in datasets){
        count_features(bam_dir=here("Data", dataset),
                    annotation_dir= here("Output"),
                    annotation_file="paired_comb_filtered.gff3",
                    output_dir=here("Output", dataset),
                    chromosome_alias_file=here("chromosome.txt"),
                    strandedness="stranded",
                    is_paired_end=TRUE,
                    output_filename = paste(dataset, "Paired", sep = "_")
                    )
}
...
for (dataset in datasets){
        count_features(bam_dir=here("Data", dataset),
                    annotation_dir= here("Output"),
                    annotation_file="single_comb_filtered.gff3",
                    output_dir=here("Output", dataset),
                    chromosome_alias_file=here("chromosome.txt"),
                    strandedness="stranded",
                    is_paired_end=FALSE,
                    output_filename = paste(dataset, "Single", sep = "_")
                    )
}
...
# read in count files for dataset
set_a<-read.delim(here("Output/PRJNA694147/PRJNA694147_Paired_Counts.csv"), header=T)
set_b<-read.delim(here("Output/PRJNA886436/PRJNA886436_Paired_Counts.csv"), header=T)
set_c<-read.delim(here("Output/PRJNA917642/PRJNA917642_Paired_Counts.csv"), header=T)
set_d<-read.delim(here("Output/PRJNA1001307/PRJNA1001307_Paired_Counts.csv"), header=T)
...
# read in count files for dataset
#single
set_e<-read.delim(here("Output/PRJNA313774/PRJNA313774_Single_Counts.csv"), header=T)
set_f<-read.delim(here("Output/PRJNA295556/PRJNA295556_Single_Counts.csv"), header=T)
...
```{r dendro_no_batch}
conditions <- read_csv(here("Data/paired_control_conditions_df.csv"), col_names=T)

# Create color mapping for each study
study_levels <- unique(conditions$study)
cols <- c("#66A61E", "#E7298A", "#E6AB02", "#1B9E77")
names(cols) <- study_levels
# Assign colors to samples based on their study
sample_colors <- cols[conditions$study]
...
conditions <- read_csv(here("Data/single_control_conditions_df.csv"), col_names=T)
